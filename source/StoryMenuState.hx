...
class StoryMenuState extends MusicBeatState
{
	...
	override function update(elapsed:Float)
	{
		...
		scoreText.text = "TOTAL SCORE:" + lerpScore;
	...
	function selectWeek()
	{
		if (!weekIsLocked(curWeek))
		{
			...
			new FlxTimer().start(1, function(tmr:FlxTimer)
			{
				...
				/*shitz = new FlxText(12, 12, 0, "Loading...", 12); //cache lol
				...
				var loadingSprite:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('funkay'));
				loadingSprite.scale.set(0.8, 0.8);
				...
				FlxTween.tween(loadingSprite, {alpha: 1}, 1);
				
				new FlxTimer().start(FlxG.random.float(2.2, 3), function(tmr:FlxTimer)
					{
