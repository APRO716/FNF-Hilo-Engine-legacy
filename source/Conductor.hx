...
class Conductor
{
	...
	public static function judgeNote(note:Note, diff:Float=0) //STOLEN FROM KADE ENGINE (bbpanzu) - I had to rewrite it later anyway after i added the custom hit windows lmao (Shadow Mario)
	{
		...
		var timingWindows:Array<Int> = [ClientPrefs.epicWindow, ClientPrefs.sickWindow, ClientPrefs.goodWindow, ClientPrefs.badWindow];
		var windowNames:Array<String> = ['epic', 'sick', 'good', 'bad'];
		//ClientPrefs.epicWindowsecond, ClientPrefs.sickWindowsecond, ClientPrefs.goodWindowsecond, ClientPrefs.badWindowsecond
